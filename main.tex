...
\noindent Fano varieites of dim $3$ with Picard rank $\rho =1$ are called \textit{prime} Fano $3$-folds and their classification was first completed by Iskovskikh using the birational method of double projection from a line in \textcolor{cyan}{Isko-1,2}. The classification was later reworked by Mukai in \textcolor{cyan{Muk89} using the biregular vector bundle method. Fano $3$-folds with $\rho\geq 2$ were all classified by Mori and Mukai in \textcolor{cyan}{refer with timeline?}. Also mention Fatighenti et.a;'s recent classification.\\
In this paper(?), we aim to understand the classification of Fano $3$-folds. 
\subsection{Potential Research?}
\\subsection{notations as needed} \textcolor{red}{ Q.2 Complete linear system, $h^i(X)$, Proposition 1.3, build upto Prop 1.6 \& as a consequence genus, and the following para- NS(X) =Pic(X) has no torsion, $\rho=b_2$, fundamental divisor $H$, index. Then Def 1.13 for degree.}\\
